...
func main() {
	...
	sampleActor := NewSampleActor("Actor")

	// Spawn the actor
	pid, err := actorSystem.SpawnActor(sampleActor)
	if err != nil {
	...
	envelopeUnstack := actor.NewEnvelope(unstackMessage, pid)
	...
	resetMessage := "reset"
	envelopeReset := actor.NewEnvelope(resetMessage, pid)
